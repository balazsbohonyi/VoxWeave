---
status: resolved
trigger: "Investigate why tauri-plugin-log is not writing to %APPDATA%\\VoxWeave\\logs\\app.log and why Rust log entries don't appear in the devtools console"
...
expected: "%APPDATA%\\VoxWeave\\logs\\app.log written; Rust logs visible in devtools console"
actual: "No logs/ folder under %APPDATA%\\VoxWeave; Rust logs absent from devtools console"
...
- hypothesis: "file write fails due to permissions"
  evidence: "logs/ dir exists and VoxWeave.log is present — plugin is writing successfully to LocalAppData"
  timestamp: 2026-03-20
...
- timestamp: 2026-03-20
  ...
  implication: "logs are being written to C:\\Users\\Balazs\\AppData\\Local\\com.balazs.bohonyi.voxweave\\logs\\VoxWeave.log — confirmed by filesystem check"

- timestamp: 2026-03-20
  checked: "tauri.conf.json line 5 — identifier: com.balazs.bohonyi.voxweave"
  found: "identifier is set, so app_log_dir() resolves to LocalAppData\\com.balazs.bohonyi.voxweave\\logs"
  implication: "the path is correct for tauri — the user's expectation (%APPDATA%\\VoxWeave\\logs) was wrong about the location"

- timestamp: 2026-03-20
  checked: "filesystem: $LOCALAPPDATA/com.balazs.bohonyi.voxweave/logs/"
  found: "directory exists, contains VoxWeave.log"
...
root_cause:
  issue_1_logdir_path: |
    NOT actually broken. The file IS being written. The path is:
      C:\Users\Balazs\AppData\Local\com.balazs.bohonyi.voxweave\logs\VoxWeave.log
    tauri-plugin-log's LogDir on Windows uses FOLDERID_LocalAppData (AppData\Local),
    not FOLDERID_RoamingAppData (AppData\Roaming). The user expected %APPDATA%\VoxWeave\logs
    but the actual path is %LOCALAPPDATA%\com.balazs.bohonyi.voxweave\logs\VoxWeave.log.
...
fix: |
  ...
  If you want logs under %APPDATA%\VoxWeave\logs specifically, replace TargetKind::LogDir
...
verification: "filesystem confirmed VoxWeave.log exists at LocalAppData path; attachConsole absence confirmed by grep across all frontend source"
files_changed: []
